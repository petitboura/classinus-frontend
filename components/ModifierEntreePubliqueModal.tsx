...
import { modifierEntreeBibliothequePublique, EntreeBibliothequePublique } from "@/lib/api";
 ...
 * déjà publié dans la bibliothèque publique, réservé au contributeur
 * d'origine (même vérification côté backend, voir
 * core/catalogue_public_publication.py::modifier_entree_publique).
 * Même squelette visuel que PopupProposerProfilPublic.tsx.
 */
export function ModifierEntreePubliqueModal({
  entree,
  onFermer,
  onModifie,
}: {
  ...
  onFermer: () => void;
  onModifie: (entree: EntreeBibliothequePublique) => void;
}) {
  ...
  async function enregistrer() {
    ...
    setErreur(null);
    try {
      const misAJour = await modifierEntreeBibliothequePublique(entree.id, { nom: nom.trim(), description: description.trim() });
      onModifie(misAJour);
      fermer();
    } catch (e) {
